...
@Component({
  selector: 'app-placement-signals',
  templateUrl: './placement-signals.component.html',
  styleUrls: ['./placement-signals.component.sass']
})
export class PlacementSignalsComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    this.ecgLabels = Array(this.ecgDataReadOnly.length).fill('');
    this.ecgChart = new Chart('ecg', {
      type: 'line',
      data: {
  ...
  addData(idx): void {
    const chart = this.ecgChart;
    const ecgData = this.ecgDataReadOnly;
    chart.data.labels.push('');
    chart.data.datasets.forEach((dataset) => {
        dataset.data.push([...ecgData][idx]);
    });
    chart.update();
  }
